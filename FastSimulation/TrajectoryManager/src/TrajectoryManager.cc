...
TrajectoryManager::reconstruct(const TrackerTopology *tTopo, RandomEngineAndDistribution const* random)
{
  ...
  for( int fsimi=0; fsimi < (int) mySimEvent->nTracks(); ++fsimi) {
    ...
    int loop = 0;
    int cyl = 0;
    int nLayers = _theGeometry->nCylinders();

    // Find the initial cylinder to propagate to.
    for(; cyl < nLayers; ++cyl){
      PP.setPropagationConditions(*_theGeometry->Cylinder(cyl));
    ...
    while(cyl < nLayers &&
      ...
      if(_theGeometry->Cylinder(cyl)->surface().mediumProperties().radLen() < 1E-10 ){\
        ++cyl;
        continue;
      }
      ...
      if(escapeBarrel){
        ++cyl;
        while(cyl < nLayers && _theGeometry->Cylinder(cyl)->forward()){
          sign=1;
          ++cyl;
        }
      ...
      TrackerLayer * layerPtr =  _theGeometry->Cylinder(cyl);

      // Define the propagation conditions
      PP.setPropagationConditions(*layerPtr, !fullPropagation);
      ...
      if(!PP.propagateToBoundSurface(*layerPtr) || PP.getSuccess()<=0){
        sign = -sign;
        ++loop;
      }
      ...
      if(PP.getSuccess() > 0 && PP.onFiducial()){
        ...
        bool saveHit = 
          ...
          layerPtr->sensitive() &&                    // Consider only sensitive layers
          PP.Perp2()>pTmin*pTmin;                    // Consider only pT > pTmin

        // Material effects are simulated there
        if(theMaterialEffects) {
          theMaterialEffects->interact(*mySimEvent,*layerPtr,PP,fsimi, random);
        }
        ...
        if(saveHit){ 
          ...
          if(layerPtr->sensitive()){
            ...
            // tracker geometry
            if(theGeomTracker){ 
              createPSimHits(*layerPtr, PP, thePSimHits[fsimi], fsimi,mySimEvent->track(fsimi).type(), tTopo);
            }
        ...
        */
        ...
        if(mySimEvent->track(fsimi).notYetToEndVertex(PP.vertex()) && !mySimEvent->filter().accept(PP)){
      ...
      if(mySimEvent->track(fsimi).notYetToEndVertex(PP.vertex())){
        ...
        if(cyl==nLayers){
          ...
          if(PP.getSuccess()==0) {
            --cyl; sign = -sign;
            PP.setPropagationConditions(*_theGeometry->Cylinder(cyl));
            PP.propagateToBoundSurface(*_theGeometry->Cylinder(cyl));

            // If there is definitely no way, leave it here.
            if(PP.getSuccess()<0) {++cyl;}
          }
...
TrajectoryManager::createPSimHits(const TrackerLayer& layer,
  ...
  const DetLayer* tkLayer = layer.getDetLayer(PP.Z());
